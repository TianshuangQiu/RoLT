...
class model:
    def __init__(self, config, data, test=False):
        ...
        self.ncm_classifier = KNNClassifier(
            self.training_opt["feature_dim"], self.training_opt["num_classes"]
        )
        ...
        if not self.test_mode:
            ...
            if self.memory["init_centroids"]:
                self.criterions["FeatureLoss"].centroids.data = self.centroids_cal(
                    self.data["train_plain"]
                )
            ...
            if config["tag"] is not None:
                ...
                self.log_file = os.path.join(
                    self.training_opt["log_dir"], f"log_{tag}.txt"
                )
            elif self.load_last:
                self.log_file = os.path.join(
                    self.training_opt["log_dir"], "log_new.txt"
                )
            elif self.load_last_v2:
                self.log_file = os.path.join(
                    self.training_opt["log_dir"], "log_new_v2.txt"
                )
            else:
            ...
            if "KNNClassifier" in self.config["networks"]["classifier"]["def_file"]:
                ...
                if not self.networks["classifier"].initialized:
                    ...
                    with open(
                        os.path.join(self.training_opt["log_dir"], "cfeats.pkl"), "wb"
                    ) as f:
        ...
        for key, val in criterion_defs.items():
            ...
            if key == "PerformanceLoss":
                ...
                if "LDAMLoss" in def_file:
                    loss_args["cls_num_list"] = self.data[
                        "train"
                    ].dataset.get_cls_num_list()
                    self.ldam = True
                else:
                    self.ldam = False
    ...
    def batch_forward(
        self, inputs, labels=None, centroids=False, feature_ext=False, phase="train"
    ):
        """
        ...
        if not feature_ext:
            ...
            self.logits, self.direct_memory_feature = self.networks["classifier"](
                self.features, centroids_
            )
    ...
    def batch_loss(self, labels, indexes):
        ...
        if "PerformanceLoss" in self.criterions.keys():
            ...
            self.loss_perf = sum(
                [
                    soft_weight
                    * self.criterions["PerformanceLoss"](
                        self.logits, soft_target[indexes]
                    )
                    for soft_weight, soft_target in zip(
                        self.soft_weights, self.soft_targets
                    )
                ]
            )
    ...
    def train(self):
        ...
        best_model_weights["feat_model"] = copy.deepcopy(
            self.networks["feat_model"].state_dict()
        )
        best_model_weights["classifier"] = copy.deepcopy(
            self.networks["classifier"].state_dict()
        )
        ncm_best_model_weights = {}
        ncm_best_model_weights["feat_model"] = copy.deepcopy(
            self.networks["feat_model"].state_dict()
        )
        ncm_best_model_weights["classifier"] = copy.deepcopy(
            self.networks["classifier"].state_dict()
        )
        best_acc = 0.0
        best_epoch = 0
        ...
        for epoch in range(self.last_epoch + 1, end_epoch + 1):
            ...
            current_cls_num_list = np.array(
                [
                    (self.current_labels == i).sum().item()
                    for i in range(self.training_opt["num_classes"])
                ]
            )

            if self.ldam:
                self.criterions["PerformanceLoss"].update_cls_num_list(
                    current_cls_num_list
                )
            ...
            if epoch == 161 and self.drw:
                ...
                self.criterions["PerformanceLoss"].weight = torch.FloatTensor(
                    per_cls_weights
                ).cuda()
                ...
                with torch.set_grad_enabled(True):
                    ...
                    if step % self.training_opt["display_step"] == 0:
                        ...
                        print_str = [
                            ...
                            "Step: %5d" % (step),
                            "Minibatch_loss_feature: %.4f" % (minibatch_loss_feat)
                            if minibatch_loss_feat
                            else "",
                            "Minibatch_loss_performance: %.4f" % (minibatch_loss_perf)
                            if minibatch_loss_perf
                            else "",
                            "Minibatch_accuracy_micro: %.4f" % (minibatch_acc),
                        ]
                        print_write(print_str, self.log_file)
        ...
        print_str = [
            "Best validation accuracy is %.4f at epoch %d" % (best_acc, best_epoch)
        ]
        print_write(print_str, self.log_file)
        print_str = [
            "NCM Best validation accuracy is %.4f at epoch %d"
            % (ncm_best_acc, ncm_best_epoch)
        ]
        print_write(print_str, self.log_file)
        ...
        __f = open(
            os.path.join(self.training_opt["log_dir"], "BestAcc_%.4f" % best_acc), "wb"
        )
        __f.close()

    def init_label_cleaning(self):
        # pdb.set_trace()
        self.noisy_labels = torch.LongTensor(
            self.data["train_plain"].dataset.targets
        ).cuda()
        self.clean_labels = torch.LongTensor(
            self.data["train_plain"].dataset.targets
        ).cuda()
        ...
        self.cls_num_list = torch.LongTensor(
            [
                (self.noisy_labels == i).sum().item()
                for i in range(self.training_opt["num_classes"])
            ]
        ).cuda()
    ...
    def label_cleaning(self, epoch):
        ...
        for _ in range(refine_times):
            ...
            refined_cfeats = self.get_knncentroids(
                feats=total_features, labels=self.noisy_labels, mask=mask
            )
        ...
        if epoch == 1:
            ...
            self.ensemble_erm_logits = (
                alpha * self.ensemble_erm_logits + (1.0 - alpha) * total_logits
            )
            self.ensemble_ncm_logits = (
                alpha * self.ensemble_ncm_logits + (1.0 - alpha) * refined_ncm_logits
            )
        self.ensemble_erm_logits = self.ensemble_erm_logits * (
            1.0 / (1.0 - alpha**epoch)
        )
        self.ensemble_ncm_logits = self.ensemble_ncm_logits * (
            1.0 / (1.0 - alpha**epoch)
        )
        ...
        def get_cls_clean_metrics(method, outputs, k):
            ...
            for shot in self.shots:
                ...
                p = (
                    (preds[p_idxs] == self.clean_labels[p_idxs].view(-1, 1))
                    .sum(dim=1)
                    .bool()
                    .float()
                    .mean()
                ...
                r = (
                    (preds[r_idxs] == self.clean_labels[r_idxs].view(-1, 1))
                    .sum(dim=1)
                    .bool()
                    .float()
                    .mean()
        ...
        shot_remains = [
            (
                (self.mask & shot[self.noisy_labels]).sum().item(),
                (self.mask & shot[self.noisy_labels] & shot[self.clean_labels])
                .sum()
                .item(),
                (shot[self.noisy_labels] & shot[self.clean_labels]).sum().item(),
            )
            for shot in self.shots
        ]
        ...
        surrogate_noise_ratio = (
            (surrogate_labels != self.clean_labels).float().mean().item()
        )
        surrogate_shot_nums = [
            (
                (shot[surrogate_labels] & (surrogate_labels != self.clean_labels))
                .sum()
                .item(),
                shot[surrogate_labels].sum().item(),
            )
            for shot in self.shots
        ]
        print_str = [
            "Surrogate noise ratio:",
            surrogate_noise_ratio,
            "\n",
            surrogate_shot_nums,
            "\n",
        ...
        if self.drw and epoch > 160:
            ...
            self.soft_targets.extend(
                [
                    torch.where(
                        self.mask,
                        self.noisy_labels,
                        torch.ones_like(self.noisy_labels).long() * i,
                    )
                    for i in range(self.training_opt["num_classes"])
                ]
            )
            ...
            self.soft_weights.extend(
                [
                    0.5 / self.training_opt["num_classes"]
                    for i in range(self.training_opt["num_classes"])
                ]
            )
            ...
            self.soft_targets = [
                torch.where(self.mask, self.noisy_labels, self.erm_preds),
            ...
            self.soft_weights.extend(
                [
                    0.2 / self.training_opt["num_classes"]
                    for i in range(self.training_opt["num_classes"])
                ]
            )
    ...
    def eval(self, phase="val", openset=False, save_feat=False):
        ...
        for inputs, labels, paths in tqdm(self.data[phase]):
            ...
            with torch.set_grad_enabled(False):
                # In validation or testing
                self.batch_forward(
                    inputs, labels, centroids=self.memory["centroids"], phase=phase
                )
        ...
        self.cls_precisions, self.cls_recalls = shot_precision_and_recall(
            preds[self.total_labels != -1], self.total_labels[self.total_labels != -1]
        )
        ...
        print_str = [
            ...
            "Phase: %s" % (phase),
            "\n\n",
            "Evaluation_accuracy_micro_top1: %.4f" % (self.eval_acc_mic_top1),
            "\n",
            "Averaged F-measure: %.4f" % (self.eval_f_measure),
            "\n",
            "Many_shot_accuracy_top1: %.4f" % (self.many_acc_top1),
            "Median_shot_accuracy_top1: %.4f" % (self.median_acc_top1),
            "Low_shot_accuracy_top1: %.4f" % (self.low_acc_top1),
            "\n",
        ]
        ...
        if "KNNClassifier" in self.config["networks"]["classifier"]["def_file"]:
            __f = open(
                os.path.join(
                    self.training_opt["log_dir"], "Acc_%.4f" % self.eval_acc_mic_top1
                ),
                "wb",
            )
            __f.close()
            ...
            if self.config["training_opt"]["dataset"] == "ImbalanceCifar10":
                print_str.append(
                    "Class Accs: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f]"
                    % tuple(self.cls_accs)
                )
                print_str.append("\n")
                print_str.append(
                    "Class Precisions: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f]"
                    % tuple(self.cls_precisions)
                )
                print_str.append("\n")
                print_str.append(
                    "Class Recalls: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f]"
        ...
        if phase == "test":
            ...
            with open(
                os.path.join(self.training_opt["log_dir"], "cls_precisions.pkl"), "wb"
            ) as f:
                pickle.dump(self.cls_precisions, f)
            with open(
                os.path.join(self.training_opt["log_dir"], "cls_recalls.pkl"), "wb"
            ) as f:
    ...
    def eval_ncm(self, phase="val", openset=False):
        ...
        # Calculate the overall accuracy and F measurement
        self.ncm_eval_acc_mic_top1 = mic_acc_cal(
            ncm_preds[total_labels != -1], total_labels[total_labels != -1]
        )
        self.ncm_eval_f_measure = F_measure(
            ncm_preds,
            total_labels,
            openset=openset,
            theta=self.training_opt["open_threshold"],
        ...
        (
            ...
            ncm_preds[total_labels != -1],
            total_labels[total_labels != -1],
            self.data["train"],
            acc_per_cls=True,
        )

        self.ncm_cls_precisions, self.ncm_cls_recalls = shot_precision_and_recall(
            ncm_preds[total_labels != -1], total_labels[total_labels != -1]
        )
        ...
        print_str = [
            ...
            "Phase: %s" % (phase),
            "\n\n",
            "Evaluation_accuracy_micro_top1: %.4f" % (self.ncm_eval_acc_mic_top1),
            "\n",
            "Averaged F-measure: %.4f" % (self.ncm_eval_f_measure),
            "\n",
            "Many_shot_accuracy_top1: %.4f" % (self.ncm_many_acc_top1),
            "Median_shot_accuracy_top1: %.4f" % (self.ncm_median_acc_top1),
            "Low_shot_accuracy_top1: %.4f" % (self.ncm_low_acc_top1),
            "\n",
        ]
    ...
    def centroids_cal(self, data, save_all=False):
        centroids = torch.zeros(
            self.training_opt["num_classes"], self.training_opt["feature_dim"]
        ).cuda()
    ...
    def load_model(self, model_dir=None):
        ...
        for key, model in self.networks.items():
            ...
            if (
                not self.test_mode
                and "DotProductClassifier" in self.config["networks"][key]["def_file"]
            ):
    ...
    def save_latest(self, epoch):
        model_weights = {}
        model_weights["feat_model"] = copy.deepcopy(
            self.networks["feat_model"].state_dict()
        )
        model_weights["classifier"] = copy.deepcopy(
            self.networks["classifier"].state_dict()
        )
        ...
        self.logger.log_model(model_dir, epoch)

    def save_model(
        self, epoch, best_epoch, best_model_weights, best_acc, centroids=None, tag="erm"
    ):
        model_states = {
            "epoch": epoch,
        ...
        if tag == "erm":
            model_dir = os.path.join(
                self.training_opt["log_dir"], f"epoch{epoch}_final_model_checkpoint.pth"
            )
        elif tag == "ncm":
            model_dir = os.path.join(
                self.training_opt["log_dir"],
                f"epoch{epoch}_ncm_final_model_checkpoint.pth",
            )

        torch.save(model_states, model_dir)
        self.logger.log_model(model_dir)
    ...
    def save_last_model(self, epoch, name="last"):
        ...
        model_dir = os.path.join(
            self.training_opt["log_dir"], f"{name}_epoch{epoch}_model_checkpoint.pth"
        )
        torch.save(model_states, model_dir)
        self.logger.log_model(model_dir)
    ...
    def load_last_model(self, name="last"):
        model_dir = os.path.join(
            self.training_opt["log_dir"], f"{name}_model_checkpoint.pth"
        )
